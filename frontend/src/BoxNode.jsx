...
function useHoverPreview() {
  ...
  const onEnter = () => {
    ...
    timerRef.current = setTimeout(() => {
      ...
      if (rect) {
        // Place below-left of the badge; clamp to viewport.
        const w = 300;
        const h = 220;
        let x = rect.right + 10;
        let y = rect.bottom + 4;
...
export default function BoxNode({ id, data, selected }) {
  ...
  return (
    <div
      className={`arch-box${selected ? " selected" : ""}${hasLink ? " has-link" : ""}${hasAttach ? " has-attach" : ""}${diffKind ? ` diff-${diffKind}` : ""}`}
      style={diffKind ? undefined : { background: bg, borderColor }}
    >
      <Handle type="target" position={Position.Left} />
      {editing ? (
        <input
          ref={inputRef}
          className="arch-box-label-input nodrag nopan"
          value={draft}
      ...
      {data._overlayMetrics && data._overlayMetrics.length > 0 && (
        <div className="arch-overlay-chips">
